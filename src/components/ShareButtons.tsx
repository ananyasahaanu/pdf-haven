...
export function ShareButtons({ title, productId }: ShareButtonsProps) {
  const { toast } = useToast();
  const url = `${SHARE_BASE}/product/${productId}`;
  const text = `Check out "${title}" on PDFStore!`;
    ...
    <div className="flex items-center gap-2">
      ...
      {shareItems.map((item) => (
        <a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer">
          <Button variant="ghost" size="icon" className={`h-8 w-8 ${item.color}`} title={item.label}>
            {item.icon}
          </Button>
        </a>
      ))}
