    ...
    <Card className="group overflow-hidden hover-lift border-border/50 bg-card">
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          {product.isBestseller && (
            <Badge className="gradient-bg text-primary-foreground border-0 text-xs">
              Bestseller
            </Badge>
          )}
          {discount > 0 && (
        ...
        <div className="absolute bottom-3 left-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
          ...
          {purchased ? (
            ...
            <Link to={isAuthenticated ? `/payment/${product.id}` : "/login"} className="flex-1">
              <Button size="sm" className="w-full gradient-bg text-primary-foreground border-0">
      ...
      <CardContent className="p-4">
        ...
        <Link to={`/product/${product.id}`}>
          <h3 className="font-display font-semibold leading-tight hover:text-primary transition-colors line-clamp-2">
            {product.title}
          </h3>
        </Link>
